...
const fetchData = async (timeRange = null) => {
  try {
    const response = await api.getNetworkMetrics(props.host, timeRange)
      ...
      datasets: [
        {
          ...
          data: response.data.map(item => item.sent_per_second)  // Already in KB from API
        },
        {
          ...chartData.value.datasets[1],
          data: response.data.map(item => item.received_per_second)  // Already in KB from API
        }
      ]
    }
    loaded.value = true
...
const handleMetricUpdate = (data) => {
  if (data.measurement === 'network' && data.host === props.host) {
    ...
    const sentInKB = typeof data.fields.sent_per_second === 'number' ? 
      data.fields.sent_per_second / 1024 : data.fields.sent_per_second;
    const receivedInKB = typeof data.fields.received_per_second === 'number' ? 
      data.fields.received_per_second / 1024 : data.fields.received_per_second;
