<template>
  ...
  const fetchData = async (timeRange = null) => {
    ...
    if (data.measurement === 'apache_raw' && data.fields.duration_per_req !== undefined) {
      pendingUpdates.value[timeLabel].reqPerSec = data.fields.duration_per_req;
      ...
      pendingUpdates.value[timeLabel].intervalReqPerSec = data.fields.interval_duration_per_req;
  ...
  const processUpdate = (timeLabel) => {
    ...
    if (timeIndex === -1) {
      ...
      // For dataset 0 (Average - duration_per_req)
      currentDatasets[0] = {
        ...currentDatasets[0],
        data: [...currentDatasets[0].data, pendingUpdate.reqPerSec / 1024]
      };
      
      // For dataset 1 (Live - interval_duration_per_req)
      currentDatasets[1] = {
        ...currentDatasets[1],
