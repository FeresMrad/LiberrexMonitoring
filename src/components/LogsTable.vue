<template>
    <div>
      <h2>Log Entries</h2>
      <a-table :columns="logColumns" :data-source="filteredLogs" :pagination="true" rowKey="log._stream_id">
        ...
        <template #bodyCell="{ column, text }">
          ...
          <template v-else-if="column.dataIndex === 'app_name'">
            <span v-if="state.searchText && state.searchedColumn === 'app_name'">
              <template v-for="(frag, i) in highlightText(text.toString(), state.searchText)" :key="i">
