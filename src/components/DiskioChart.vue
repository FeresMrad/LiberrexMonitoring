<template>
  ...
  const fetchData = async (timeRange = null) => {
    ...
    let query = `SELECT disk_read_per_second, disk_write_per_second FROM disk WHERE host='${props.host}'`
        ...
        datasets: [
          {
            ...
            data: data.map(item => item[1] / 1024)  // 'disk_read' data in KB
          },
          {
            ...chartData.value.datasets[1],
            data: data.map(item => item[2] / 1024)  // 'disk_write' data in KB
          }
        ]
      }
      loaded.value = true
  ...
  const handleNewData = (data) => {
    if (data.measurement === 'disk' && data.host === props.host) {
      ...
      const diskReadInKB = data.fields.disk_read_per_second / 1024;
      const diskWriteInKB = data.fields.disk_write_per_second / 1024;
