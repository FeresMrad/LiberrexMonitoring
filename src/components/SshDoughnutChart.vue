<template>
  ...
  const props = defineProps({
    ...
    connectionType: {
      type: String,
      required: true,
      validator: (value) => ['accepted', 'failed'].includes(value)
    },
    timeRange: {
      type: [String, Object],
      default: '60m'
  ...
  // Chart title based on connectionType
  const chartTitle = computed(() => {
    return props.connectionType === 'accepted' 
      ? 'Accepted Connections by User' 
  ...
  // Function to fetch SSH data by user
  const fetchData = async () => {
    ...
    try {
      // Select API method based on connectionType
      const apiMethod = props.connectionType === 'accepted' 
        ? api.getSshAcceptedUsers 
        : api.getSshFailedUsers;
      
      // Call API for connections
      const response = await apiMethod(props.host, props.timeRange)
  ...
  const chartData = computed(() => {
    ...
    return {
      ...
      datasets: [
        {
          label: props.connectionType === 'accepted' 
            ? 'Accepted SSH Logins by User' 
            : 'Failed SSH Logins by User',
          data,
